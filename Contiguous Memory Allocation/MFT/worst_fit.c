...
int alloc(int pid, Partition part[], Process pro[]){
    int worst_partid = -1;
    int worst_size = INF_NEG;

    loop(j,partition_count){
        if(part[j].allocated==false && pro[i].req<=part[j].size && part[j].size > worst_size){
            worst_size = part[j].size;
            worst_partid = j;
    ...
    int j = worst_partid;
    if(j !=-1 ){
        //i is pid stored globally TODO : change 
...
void worst_fit(Partition part[], Process pro[]){
    println();
